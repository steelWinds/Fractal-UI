<script context="module" lang="ts">
	let currentFilterType: string = '';
</script>

<script lang="ts">
	import type { Writable } from 'svelte/store';

	import Button from '$lib/UI/Button.svelte'
	import { getContext } from 'svelte';

	const outState = getContext<Writable<any>>('out-filters');

	export let type: string = '';
	export let activeClass: string = '';
	export let disableClass: string = '';
	export let state: string = '';

	const trueState = `up_${type}`
	const falseState = `down_${type}`

	$: isActive = type === $outState.currentFilter;

	const switchState = () => {
		state = state === trueState ? falseState : trueState;
	}

	console.log($outState.currentFilter)

	const setCurrentFilterType = (typeFilter: string) => {
		currentFilterType = typeFilter;
			
		$outState.currentFilter = typeFilter
	}
</script>

<Button
	on:click={() => {
			setCurrentFilterType(type)
			switchState()
		}
	}
	class={`
		tw-flex
		tw-flex-cols
		tw-items-center
	`}
>
	<slot {isActive}></slot>

	<svg 
		width="24" 
		height="24" 
		class:tw-rotate-180={state === falseState}
		class={`
			tw-transition-all tw-duration-250 ${
				isActive ? activeClass : disableClass
			}
		`}
		viewBox="0 0 24 24" 
		xmlns="http://www.w3.org/2000/svg"
	>
		<path d="M12 4L11.2929 3.29289L12 2.58579L12.7071 3.29289L12 4ZM13 19C13 19.5523 12.5523 20 12 20C11.4477 20 11 19.5523 11 19L13 19ZM5.29289 9.29289L11.2929 3.29289L12.7071 4.70711L6.70711 10.7071L5.29289 9.29289ZM12.7071 3.29289L18.7071 9.29289L17.2929 10.7071L11.2929 4.70711L12.7071 3.29289ZM13 4L13 19L11 19L11 4L13 4Z" />
	</svg>
</Button>
